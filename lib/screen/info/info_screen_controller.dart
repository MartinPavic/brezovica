  ...
  Future<List<Post>> getPosts(SearchParameters searchParameters) {
    return contentfulService
        .getEntryCollection<Post>(
            searchParameters, (json) => Post.fromJson(json as Map<String, dynamic>),)
        .map(
          (postsCollection) => postsCollection.items.map((e) => e.fields).toList(),
        ).run();
  }
}
...
final getPostsFutureProvider = FutureProvider.family<List<Post>, SearchParameters>((ref, searchParameters) {
